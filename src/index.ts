...
export const parseEnv = (env: string | undefined): string[] => {
	...
	return matches.map(
		(match) => match.slice(1).find((capture) => capture !== undefined) || "",
	);
};

export default {
	async email(message: ForwardableEmailMessage, env): Promise<void> {
...
const formatAddresses = (
	addresses: PostalMime.Address[] | undefined,
): string => {
...
const formatSingleAddress = (
	address: PostalMime.Address | undefined,
): string => {
	if (!address) {
		return "N/A";
	}
...
const isUrlLike = (s: string): boolean =>
	/^(https?:\/\/|https?:%2F%2F|www\.)/i.test(s);
...
const findUrlRanges = (text: string): UrlRange[] => {
	...
	while (m !== null) {
		...
		if (urlEnd > urlStart) {
			...
			if (
				(wrappedByParen || wrappedByBracketParen) &&
				end < text.length &&
				text[end] === ")"
			) {
				end += 1;
...
export const fixMarkdownFormatting = (markdown: string): string => {
	...
	fixed = fixed.replace(/(\]\([^)]+\))(\P{ASCII})/gu, "$1 $2");
...
const getAttr = (
	el: HTMLElementWithAttributes | null,
	name: string,
): string => {
...
const pickImgUrl = (img: HTMLElementWithAttributes | null): string => {
 ...
 */
const pickRootNodeForTurndown = (
	document: DocumentLike,
): HTMLElementWithAttributes | null => {
	const candidates: (HTMLElementWithAttributes | null)[] = [];
	if (document?.body) {
	...
	candidates.push(document as HTMLElementWithAttributes);

	const score = (node: HTMLElementWithAttributes | null): number => {
		if (!node) {
			return 0;
		}
		const textLen = String(node.textContent || "").trim().length;
		const imgCount = node.querySelectorAll
			? node.querySelectorAll("img").length
			: 0;
		const aCount = node.querySelectorAll
...
export const htmlToMarkdown = (html: string): string => {
	...
	td.addRule("imgKeep", {
		filter: (node: HTMLElementWithAttributes) => {
			return node.nodeName === "IMG";
		},
		replacement: (_content, node: HTMLElementWithAttributes) => {
	...
	td.addRule("aKeep", {
		...
		replacement: (content: string, node: HTMLElementWithAttributes) => {
...
export const chunkForDiscord = (content: string, limit: number): string[] =>
	chunkText(content, limit);
